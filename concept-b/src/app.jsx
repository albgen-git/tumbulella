...
function App() {
  ...
  async function playSequence(item, session) {
    ...
    // La fetch parte SUBITO (mai bloccata dal filler): il backend su Render
    // va in sleep dopo un po' di inattività e la prima richiesta di una
    // nuova partita può metterci fino a un minuto a risvegliarlo.
    const narrationPromise = window.fetchNarration(item.n, lastSentencesRef.current, previousNumbers, lang);
    ...
    if (isFirstTurn && !mutedRef.current) {
      // Riempitivo letto in locale (Web Speech, mai Gemini) in parallelo
      // all'attesa del backend, così il primo numero non resta muto.
